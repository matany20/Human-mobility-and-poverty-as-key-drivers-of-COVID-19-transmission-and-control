...
def shrink_array_sum(mapping_dic, array_to_shrink):
    """The function gets mapping_dic - indices to assign in the shrunk array (with key granularity based on
    array_to_expand), and array_to_shrink - the shrunk array's values will be based on this array. Returns
    and shrunk array shape (len(mapping_dic),1) based on the array_to_shrink"""
...
def calculate_force_matriceis(C,Ie,Is,Ia,t,alpha):
    ...
    force_out = (C['work_inter'][t].T.dot((Ie[inter_risk_dict['Intervention', 'Low']] + Ie[inter_risk_dict['Intervention', 'High']]) * alpha + \
                 ...
                 C['leisure_non'][t].T.dot((Ie[inter_risk_dict['Non-intervention', 'Low']] + Ie[inter_risk_dict['Non-intervention', 'High']]) * alpha + \
                                            Is[inter_risk_dict['Non-intervention', 'Low']] + Is[inter_risk_dict['Non-intervention', 'High']] + \
                                            Ia[inter_risk_dict['Non-intervention', 'Low']] + Ia[inter_risk_dict['Non-intervention', 'High']]))
    return {'out':force_out, 'home':force_home}