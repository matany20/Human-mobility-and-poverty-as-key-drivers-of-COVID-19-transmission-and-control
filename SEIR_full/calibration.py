...
def errorfunc(tpl, data, f, season_length, eps, alpha):
    # setting parameters
    # beta_home = tpl[5]
    beta = np.array([tpl[0], tpl[0], tpl[1], tpl[2], tpl[2], tpl[3], tpl[3], tpl[4], tpl[4]])

    # Run model with given parameters
    model_result = run_model(beta_home=0.38/9., beta_j=beta, eps=eps, f=f, alpha=alpha, days_in_season=season_length)
    ...
    for i, age_group in enumerate(age_dict.keys()):
        model_results_cal[:, i] = new_cases_model[:, age_dict[age_group]].sum(axis=1)
