...
def run_global_policy(ind, model, policy_params, sim_time, is_pop):
	...
	for period in range(int(float(sim_time)/policy_params['policy_period'])):
		...
		C_apply, stay_home_idx_apply = apply_policy(ind, regions_applied,
													C_stop,
													stay_home_idx_stop,
													C_free,
													stay_home_idx_free,
...
def evaluate_policy(ind,policy_params, horizon, return_data=False):
    ...
    for key in cal_parameters.keys():
        ...
        for i in range(horizon):
            ...
            if 'SeasonalBeta' in policy_params and policy_params['SeasonalBeta']:
                beta_factor = SeasonalBetaFactor(datetime.timedelta(days= i*policy_period) + start_inter)
            else:
                beta_factor = 1
...
def simple_policy_function(
	...
	for j, age in enumerate(ind.age_dict.keys()):
		...
		Vals[2][j] = (
						 res_mdl['Hosp_latent'][-1][ind.age_dict[age]]).sum() + \
					 (res_mdl['H'][-1][ind.age_dict[age]]).sum()
		Vals[3][j] = (res_mdl['Vents_latent'][-1][
			ind.age_dict[age]]).sum() + \
					 (res_mdl['Vents'][-1][ind.age_dict[age]]).sum()

	global_val_for_trhsh = (
		np.multiply(Vals, policy_params['weight_matrix'])).sum()*is_pop

	if ('global_decision' in policy_params) and (
	...
	elif ('global_decision' in policy_params) and \
		...
		if (global_val_for_trhsh > policy_params['threshold']):
			...
			for i, region in enumerate(ind.region_dict.keys()):
				...
				for j, age in enumerate(ind.age_dict.keys()):
					...
					Vals[2][j] = (res_mdl['Hosp_latent'][-1][
...
def apply_policy(ind,regions_applied,C_inter,stay_home_idx_inter,C_no_inter,stay_home_idx_no_inter,t_range):
    ...
    regions_applied : list of regions in a lockdown
     """
    C_apply = {}
    for val in C_no_inter.keys():
        C_apply[val] = []
        for i,t in enumerate(t_range):
            C_apply[val].append(C_no_inter[val][i])
    ...
    for key in C_inter.keys():
        ...
        C_mat = csr_matrix(mdl.isolate_areas(ind,(C_no_inter[key][0]).todense(), (C_inter[key][0]).todense(), regions_applied))
    ...
    for key in stay_home_idx_inter.keys():
        for key2 in stay_home_idx_inter[key].keys():
            stay_home_idx = mdl.isolate_areas_vect(ind,stay_home_idx_applied[key][key2][0],
                            stay_home_idx_inter[key][key2][0],
                            regions_applied)
            for t in t_range:
                stay_home_idx_applied[key][key2][t] = stay_home_idx
